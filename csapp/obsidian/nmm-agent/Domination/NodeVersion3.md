...
### [심화] 웹훅(Webhook) 요청 흐름 예시

Dify에서 HTTP Request 노드를 사용하여 외부 시스템(예: Slack)의 웹훅을 호출할 때의 흐름은 다음과 같습니다.

  

1. **Dify 클라이언트 (사용자)** -> **Dify 서버**

* 사용자가 워크플로우 실행을 요청합니다. (예: "실행" 버튼 클릭)

* 이 시점에서는 Dify 서버에게 작업을 시작하라는 명령만 전달됩니다.

2. **Dify 서버 (엔진)**

* 워크플로우를 순서대로 실행하다가 **HTTP Request 노드**에 도달합니다.

* 이 노드가 "외부로 요청을 보내야 함"을 인지하고 실행됩니다.

3. **Dify 서버** -> **외부 서버 (예: Slack)**

* Dify 서버가 직접 **웹훅 URL**로 HTTP 요청을 전송합니다.

* 클라이언트가 직접 외부 서버와 통신하는 것이 아니라, **Dify 서버가 중계자(Sender)**가 되어 대신 요청을 보내는 구조입니다.
