...
이 문제는 삼각 달팽이 형태의 배열을 채우는 문제입니다. 크게 두 가지 접근 방식을 고려할 수 있습니다. 첫 번째는 `while`문을 사용하여 직접 방향 전환 조건을 검사하면서 채우는 방식(충돌 감지 방식)이고, 두 번째는 배열을 채우는 패턴의 규칙성을 활용하여 중첩 `for`문을 사용하는 최적화된 방식입니다.

### 1. 문제 접근 핵심 개념

#### 1) 삼각형 배열의 구조

- 밑변과 높이가 `n`인 직각 삼각형 형태입니다.
- 전체 채워야 할 원소의 총 개수는 `1 + 2 + ... + n`개입니다. (공식: `n * (n + 1) // 2`)
- 예를 들어 `n = 4`라면 총 10개의 칸을 채워야 합니다.

#### 2) 이동 방향 (반시계 달팽이)

이 문제는 일반적인 2차원 배열의 달팽이 회전과 달리 삼각형 내부에서 **반시계 방향**으로 이동합니다.
이동 순서와 좌표(x, y)의 변화는 다음과 같은 3가지 패턴이 반복됩니다.

1. 아래 방향: `y + 1`
2. 오른쪽 방향: `x + 1`
3. 왼쪽 위 대각선 방향: `y - 1`, `x - 1`

#### 3) 방향 전환 기준 (충돌 감지 방식의 경우)

다음 위치로 이동할 때 아래 조건 중 하나를 만나면 방향을 전환합니다.

- 배열의 범위를 벗어나는 경우
- 이미 값이 할당되어 있는 경우 (0이 아닌 경우)

### 2. 초기 1차 구현 방식 (충돌 감지 및 while문 활용)

반복문을 통해 한 칸씩 이동할 때마다 범위를 검사하고 방향을 갱신하는 직관적인 방법입니다.

#### 1) 배열 생성 및 초기화

N×N 크기의 정사각형 배열이 아닌, 필요한 만큼만(삼각형 형태로) 배열을 생성하여 메모리를 절약합니다.
...
방향 벡터를 정의하고 `while`문 내에서 전체 개수만큼 반복하여 숫자를 채워 넣습니다.
진행 방향에 있는 다음 위치가 유효하면 그대로 진행하고, 그렇지 않다면 방향 각도를 회전시켜 갱신합니다.

```python
dx = [0, 1, -1]
dy = [1, 0, -1]
angle = 0  # 0: 아래, 1: 오른쪽, 2: 왼쪽 위

size = n * (n + 1) // 2
num = 1
y, x = -1, 0
# while 문으로 size 개수만큼 루프 진행 및 범위 체크
```

- **문제점**: 이 방식은 매번 이동할 때마다 경계 범위나 값 존재 여부를 확인하는 추가 연산(충돌 검사)과 조건 분기가 필요해 코드가 길어지고 속도가 상대적으로 느릴 수 있습니다.

### 3. 구조 최적화 방식 (이중 for문 활용)

규칙성을 파악해 조건 분기 없이 채워나가는 깔끔한 최적화 방식입니다.
`n`이 주어졌을 때 각 방향으로 이동해야 하는 횟수가 바깥부터 안쪽으로 1씩 줄어든다는 패턴(예: `n = 4`이면 4번 → 3번 → 2번 → 1번)을 이용합니다.

#### 1) 핵심 반복 논리 및 방향 제어

이동 횟수의 규칙을 그대로 `for`문에 적용하며, 바깥쪽 루프의 인덱스를 3으로 나눈 나머지를 이용하여 방향을 결정합니다. 방향 전환 로직과 경계 검사가 필요 없어집니다.

```python
# angle 변수 대신 바깥 문자 i를 활용한 방향 결정 (i % 3)
# 0 -> 아래, 1 -> 오른쪽, 2 -> 왼쪽 위
for i in range(n):
    for j in range(i, n):
        if i % 3 == 0:
...
완성된 2차원 배열(삼각형 배열)을 문제의 요구사항에 맞게 1차원 리스트로 펼쳐줍니다. `reduce`나 외부 라이브러리를 사용할 수도 있지만, 리스트 컴프리헨션 구문이 간결하며 파이썬에서 권장되는 효율적인 방식입니다.

```python
return [i for row in res for i in row]
```
...
| 구분            | 1차 구현                         | 최적화 구현                             |
| --------------- | -------------------------------- | --------------------------------------- |
| 반복 제어       | `while`문과 매번 개수 확인 계산  | 이중 `for`문으로 횟수 자동 제어         |
| 방향 전환       | 배열 충돌을 감지한 후 각도 변경  | 반복 인덱스의 모듈러 연산(`i % 3`) 활용 |
| 충돌(조건) 검사 | 필요함 (속도 저하 요인)          | 불필요함                                |
| 코드 스타일     | 길고 복잡함                      | 직관적이고 짧음                         |
| 사고 난이도     | 상대적으로 높음                  | 낮음 (패턴 발견 시)                     |
| 실행 속도       | 분기 연산이 많아 상대적으로 느림 | 조건 검사가 제외되어 더 빠름            |

### 5. 중요한 개념 요약

- **메모리 절약**: 삼각형 형태의 배열은 굳이 정사각형 크기의 `N × N`으로 구성할 필요 없이, `[[0] * i for i in range(1, n + 1)]`처럼 행마다 크기를 가변적으로 생성하여 불필요한 메모리 낭비를 줄입니다.
- **반복 구조의 규칙성 우대**: 고정된 형태로 방향이 전환되는 상황은 충돌(경계) 감지 로직으로 해결하는 것보다, 전체적인 이동 횟수의 규칙과 패턴을 파악해 중첩 반복문으로 처리하는 것이 예외 상황 발생이 적고 훨씬 깔끔합니다.
- **다차원 배열의 평탄화(Flatten)**: 파이썬에서 2차원 이상의 배열을 1차원 리스트로 펼칠 때는 `[val for row in arr for val in row]`와 같은 리스트 컴프리헨션 구문을 적극 활용하면 직관적이고 가독성 높은 코드를 작성할 수 있습니다.
